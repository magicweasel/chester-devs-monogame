...
namespace BestGameEver.Droid.ParticleSystem
{
	...
	public abstract class ParticleSystem : DrawableGameComponent
	{
		...
		public void LoadContent(Texture2D texture)
		{
			...
			this.origin.X = this._texture.Width / 2;
			this.origin.Y = this._texture.Height / 2;
		...
		public void AddParticles(Vector2 where)
		{
			...
			int numParticles =
				RandomHelper.Random.Next(this.minNumParticles, this.maxNumParticles);
		...
		protected virtual void InitializeParticle(Particle p, Vector2 where)
		{
			...
			Vector2 direction = (this._useDirection) ? this._direction : this.PickRandomDirection();

			// pick some random values for our particle
			float velocity =
				RandomHelper.RandomBetween(this.minInitialSpeed, this.MaxInitialSpeed);
			float acceleration =
				RandomHelper.RandomBetween(this.minAcceleration, this.maxAcceleration);
			float lifetime =
				RandomHelper.RandomBetween(this.minLifetime, this.maxLifetime);
			float scale =
				RandomHelper.RandomBetween(this.minScale, this.maxScale);
			float rotationSpeed =
				RandomHelper.RandomBetween(this.minRotationSpeed, this.maxRotationSpeed);
		...
		public void Draw(SpriteBatch spriteBatch, GameTime gameTime)
		{
			...
			foreach (var p in this.particles)
			{
				...
				spriteBatch.Draw(this._texture, p.Position, null, color,
					p.Rotation, this.origin, scale, SpriteEffects.None, 0.0f);
			}
		...
		public override void Draw(GameTime gameTime)
		{
			...
			// tell sprite batch to begin, using the spriteBlendMode specified in
			// initializeConstants
			this._spriteBatch.Begin(SpriteSortMode.Deferred, this.blendState);
			...
			foreach (var p in this.particles)
			{
				...
				this._spriteBatch.Draw(this._texture, p.Position, null, color,
					p.Rotation, this.origin, scale, SpriteEffects.None, 0.0f);
			}
